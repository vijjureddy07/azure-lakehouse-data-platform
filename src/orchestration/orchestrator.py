...
class LakeflowLocalOrchestrator:
    ...
    def run(self, spark: SparkSession | None = None) -> JobRunAudit:
        ...
        DAG Flow:
            ...
            ➔ silver_transformation & hard quarantine gate (retries: 0 for data quality)
        ...
        try:
            ...
            # Task 3: Silver Transformation & Hard Quarantine Gate
            t3_ok = self._execute_task_step(
                ...
                retry_policy=RetryPolicy(max_retries=0, retryable_classifications={FailureClassification.TRANSIENT}),
            )
            if not t3_ok:
            ...
            audit_box: list[JobRunAudit] = []

            def _publish_summary_action():
                rec = execute_publish_run_summary_task(
                    spark=active_spark,
                ...
                audit_box.append(rec)
                return rec

            self._execute_task_step(
                task_name="publish_run_summary",
                task_func=_publish_summary_action,
                retry_policy=RetryPolicy(max_retries=1),
            )
            audit_record = audit_box[0] if audit_box else None
